...
def get_book_copy_service(broker: BookCopyBroker = Depends(get_book_copy_broker)) -> BookCopyService:
    return BookCopyService(broker)

def get_course_service(broker: CourseBroker = Depends(get_course_broker)) -> CourseService:
