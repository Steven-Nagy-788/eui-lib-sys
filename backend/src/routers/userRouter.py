...
async def login(
    credentials: UserLogin, service: UserService = Depends(get_user_service)
):
    ...
    token_data = {"id": str(user.id), "role": user.role, "exp": expires}
...
async def update_user(
    user_id: UUID, user: UserUpdate, service: UserService = Depends(get_user_service)
):
...
async def add_to_blacklist(
    ...
    user = await service.ModifyUser(
        user_id, UserUpdate(is_blacklisted=True, blacklist_note=reason)
    )
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def remove_from_blacklist(
    ...
    user = await service.ModifyUser(
        user_id, UserUpdate(is_blacklisted=False, blacklist_note=None)
    )
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
