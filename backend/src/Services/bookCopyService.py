...
class BookCopyService:
    ...
    async def RetrieveAllCopies(
        self, skip: int = 0, limit: int = 10
    ) -> List[BookCopyResponse]:
        """Get all book copies with pagination"""
        copies = await self.broker.SelectAllCopies(skip=skip, limit=limit)
        return [BookCopyResponse(**copy) for copy in copies]

    async def RetrieveCopiesByBookId(self, book_id: UUID) -> List[BookCopyResponse]:
        """Get all copies of a specific book"""
        copies = await self.broker.SelectCopiesByBookId(book_id)
        return [BookCopyResponse(**copy) for copy in copies]

    async def RetrieveCopiesByBookIdWithBorrowerInfo(
        self, book_id: UUID, available_only: bool = False
    ) -> List[BookCopyWithBorrowerInfo]:
        """Get copies with borrower information"""
        copies = await self.broker.SelectCopiesByBookIdWithBorrowerInfo(
            book_id, available_only
        )
        return [BookCopyWithBorrowerInfo(**copy) for copy in copies]

    async def RetrieveAvailableCopiesByBookId(
        self, book_id: UUID
    ) -> List[BookCopyResponse]:
        """Get all available copies of a specific book"""
        copies = await self.broker.SelectAvailableCopiesByBookId(book_id)
        return [BookCopyResponse(**copy) for copy in copies]

    async def RetrieveCopyById(self, copy_id: UUID) -> Optional[BookCopyResponse]:
        """Get a specific copy by ID"""
        copy_data = await self.broker.SelectCopyById(copy_id)
        return BookCopyResponse(**copy_data) if copy_data else None

    async def RetrieveCopyByAccessionNumber(
        self, accession_number: int
    ) -> Optional[BookCopyResponse]:
        """Get a copy by barcode/accession number"""
        copy_data = await self.broker.SelectCopyByAccessionNumber(accession_number)
        return BookCopyResponse(**copy_data) if copy_data else None

    async def RetrieveCopyStats(self, book_id: UUID) -> dict:
        """Get statistics for book copies"""
        return await self.broker.CountCopiesByBookId(book_id)

    async def AddSingleCopy(self, copy: BookCopyCreate) -> BookCopyResponse:
        """Create a single book copy manually"""
        copy_data = copy.model_dump()
        # Convert UUID to string for JSON serialization
        copy_data["book_id"] = str(copy_data["book_id"])
        created_copy = await self.broker.InsertCopy(copy_data)
        return BookCopyResponse(**created_copy)

    async def AddBulkCopies(
        self, book_id: UUID, quantity: int, reference_percentage: int = 30
    ) -> List[BookCopyResponse]:
        """
        ...
        for _ in range(reference_count):
            copies_data.append(
                {"book_id": str(book_id), "is_reference": True, "status": "available"}
            )

        # Add circulating copies
        for _ in range(circulating_count):
            copies_data.append(
                {"book_id": str(book_id), "is_reference": False, "status": "available"}
            )

        # Insert all at once
        created_copies = await self.broker.InsertCopiesBulk(copies_data)
        return [BookCopyResponse(**copy) for copy in created_copies]

    async def ModifyCopy(
        self, copy_id: UUID, copy_update: BookCopyUpdate
    ) -> Optional[BookCopyResponse]:
        """Update a book copy (status, reference flag, etc.)"""
        existing_copy = await self.broker.SelectCopyById(copy_id)
        if not existing_copy:
            return None

        update_data = copy_update.model_dump(exclude_unset=True)
        updated_copy = await self.broker.UpdateCopy(copy_id, update_data)
        return BookCopyResponse(**updated_copy) if updated_copy else None

    async def ModifyCopyStatus(
        self, copy_id: UUID, status: BookStatus
    ) -> Optional[BookCopyResponse]:
        """Quick status update (available, maintenance, lost)"""
        return await self.ModifyCopy(copy_id, BookCopyUpdate(status=status))

    async def RemoveCopy(self, copy_id: UUID) -> bool:
        """Delete a book copy"""
        return await self.broker.DeleteCopy(copy_id)
