...
from ..Models.Courses import CourseCreate, CourseResponse, CourseUpdate, EnrollmentCreate
...
class IUserService(ABC):
    ...
    @abstractmethod
    async def RetrieveAllUsers(self, skip: int = 0, limit: int = 50) -> List[UserResponse]:
    ...
    @abstractmethod
    async def ModifyUser(self, user_id: UUID, user: UserUpdate, current_user_id: UUID, is_admin: bool) -> Optional[UserResponse]:
        """Update a user"""
        pass
    ...
    @abstractmethod
    async def AddToBlacklist(self, user_id: UUID, reason: str) -> Optional[UserResponse]:
...
class ILoanService(ABC):
    ...
    @abstractmethod
    async def RetrieveAllLoans(self, skip: int = 0, limit: int = 50) -> List[LoanResponse]:
    ...
    @abstractmethod
    async def RetrieveLoansByStatus(self, status: str, skip: int = 0, limit: int = 50) -> List[LoanWithBookInfo]:
    ...
    @abstractmethod
    async def CancelLoan(self, loan_id: UUID, user_id: UUID, is_admin: bool) -> Optional[LoanResponse]:
...
class ICourseService(ABC):
    ...
    @abstractmethod
    async def RetrieveAllCourses(self, skip: int = 0, limit: int = 50) -> List[CourseResponse]:
    ...
    @abstractmethod
    async def ModifyCourse(self, course_id: UUID, course: CourseUpdate) -> Optional[CourseResponse]:
        """Update a course"""
        pass
    ...
    @abstractmethod
    async def EnrollStudent(self, course_id: UUID, enrollment: EnrollmentCreate) -> dict:
