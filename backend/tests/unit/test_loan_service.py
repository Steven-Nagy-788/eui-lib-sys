...
class TestLoanService:
    ...
    @pytest.fixture
    def service(self, mock_loan_broker, mock_user_broker, mock_copy_broker, mock_course_broker):
        """Create LoanService instance with mocked brokers"""
        return LoanService(mock_loan_broker, mock_user_broker, mock_copy_broker, mock_course_broker)
    
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_get_all_loans_success(self, service, mock_loan_broker, sample_loan_dict):
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_get_loan_by_id_found(self, service, mock_loan_broker, sample_loan_dict):
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_get_loans_by_status(self, service, mock_loan_broker, sample_loan_dict):
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_create_loan_request_success(self, service, mock_loan_broker, mock_user_broker, mock_copy_broker, mock_course_broker, sample_loan_dict):
        """Test successful loan request creation"""
        copy_id = uuid4()
        user_id = uuid4()
        book_id = str(uuid4())
        mock_user_broker.SelectUserById.return_value = {"id": str(user_id), "is_blacklisted": False, "role": "student"}
        mock_loan_broker.SelectLoanPolicy.return_value = {"max_books": 5, "loan_duration_days": 14, "loan_days": 14}
        mock_copy_broker.SelectCopyById.return_value = {"status": "available", "is_reference": False, "book_id": book_id}
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_create_loan_request_copy_not_available(self, service, mock_loan_broker, mock_user_broker, mock_copy_broker):
        """Test loan request fails when copy is not available"""
        copy_id = uuid4()
        user_id = uuid4()
        mock_user_broker.SelectUserById.return_value = {"id": str(user_id), "is_blacklisted": False, "role": "student"}
        mock_loan_broker.SelectLoanPolicy.return_value = {"max_books": 5, "loan_duration_days": 14}
        mock_copy_broker.SelectCopyById.return_value = {"status": "checked_out", "is_reference": False, "book_id": str(uuid4())}
        mock_loan_broker.SelectActiveLoansByUser.return_value = []
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_approve_loan_success(self, service, mock_loan_broker, mock_user_broker, mock_copy_broker, mock_course_broker, sample_loan_dict):
        """Test successful loan approval"""
        loan_id = uuid4()
        copy_id = str(uuid4())
        user_id = str(uuid4())
        book_id = str(uuid4())
        pending_loan = {**sample_loan_dict, "status": "pending", "user_id": user_id, "copy_id": copy_id}
        approved_loan = {**sample_loan_dict, "status": "pending_pickup"}
        
        mock_loan_broker.SelectLoanById.return_value = pending_loan
        mock_user_broker.SelectUserById.return_value = {"id": user_id, "role": "student"}
        mock_copy_broker.SelectCopyById.return_value = {"book_id": book_id}
        mock_course_broker.SelectCoursesByBook.return_value = []
        mock_course_broker.SelectEnrollmentsByStudent.return_value = []
        mock_loan_broker.SelectLoanPolicy.return_value = {"loan_duration_days": 14, "loan_days": 14}
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_checkout_loan_success(self, service, mock_loan_broker, mock_copy_broker, sample_loan_dict):
        """Test successful loan checkout"""
        loan_id = uuid4()
        pending_loan = {**sample_loan_dict, "status": "pending_pickup", "copy_id": str(uuid4())}
